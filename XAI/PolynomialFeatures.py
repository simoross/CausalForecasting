...
def Polynomial(dataframe, features, target, grade):
    r"""
    Preprocesses the data, trains a Polynomial Regression model, and returns predictions on the test set.
    ...
    - grade (int): The degree of the polynomial for the regression model.
    ...
    6. Initializes a Polynomial Regression pipeline:
       - The pipeline first transforms the input data into polynomial features of degree 'grade'.
       - Then, it fits a linear regression model to these transformed features.
    7. Trains the Polynomial Regression model using the training data (X_train and y_train).
    ...
    Y = self_dataframe[self_target].to_numpy()
    ...
    reg = make_pipeline(PolynomialFeatures(grade), LinearRegression())
